...
class customDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self,index):
        ...
        # transform image
        sample={"imgs":img,"anno":my_annotation}
...
def get_transform():
    custom_trans=[]
    custom_trans.append(torchvision.transforms.ToTensor())
    return torchvision.transforms.Compose(custom_trans)

def collater(data):
    imgs=[s['imgs'] for s in data]
    annotations=[s['annotations'] for s in data]
    ...
    max_num_annots=max(annotation.shape[0] for annotation in annotations)
    if max_num_annots > 0:
